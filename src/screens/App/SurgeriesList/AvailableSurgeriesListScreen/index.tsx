...
export default function AvailableSurgeriesListScreen() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    onSnapshot(
      query(collection(db, 'surgeries'), orderBy('date', 'asc'), limit(10)),
      ({ docs }) => {
        ...
        setSurgeries(
          surgeriesAux.filter(
            ({ participantsAssistants, authorId }) =>
              !participantsAssistants.includes(user?.uid as string) &&
              authorId !== user?.uid,
          ),
        )
      },
